...
public class DateTimeAgeService {
    ...
    public String getTimeDifference(LocalTime startTime, LocalTime endTime) {
        ...
        return hours + " hours, " + minutes + " minutes " + seconds + " seconds , and nano second " + nanoseconds;
    }
    ...
    /*
     * Converting the LocalDate to LocalDateTime using atStartOfDay() method. This
     * method adds midnight time (start
    ...
    public String getMonthName(int monthNumber) {
        return Month.of(monthNumber).name().trim().toLowerCase().substring(0, 1).toUpperCase()
                + Month.of(monthNumber).name().trim().toLowerCase().substring(1);
    }
    ...
    public boolean createdAtIsValidateWithGivenDateRange(LocalDate from, LocalDate to, LocalDateTime givenDate) {
        ...
        if (fromFormat.isAfter(givenDate) || toFormat.isBefore(givenDate) || fromFormat.isEqual(givenDate)
                || toFormat.isEqual(givenDate)) {
            valide = true;
        }
